...
public class ListaDeEnterosEnlazada extends ListaDeEnteros {
	...
	@Override
	public void imprimirAlReves(ListaDeEnteros L, Integer elem) {
		if (! L.fin()) {
			imprimirAlReves(L, L.proximo());
		}
		System.out.println(elem);
	}
	public void imprimir (ListaDeEnteros L, Integer elem) {
		System.out.println(elem);
		if (!L.fin()) 
			imprimir(L, L.proximo());
	}
	public ListaDeEnterosEnlazada sucesion (int n, ListaDeEnterosEnlazada L) {
		L.agregarFinal(n);
		if (n!=1) { 
			if (n%2==0) {
				n=n/2;
